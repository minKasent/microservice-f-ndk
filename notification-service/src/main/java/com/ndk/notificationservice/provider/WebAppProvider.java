...
@Slf4j
public class WebAppProvider implements NotificationProvider {
  ...
  public void send(NotificationMessage message) {
    RateLimiter rateLimiter = rateLimiterRegistry.rateLimiter("webapp");
    ...
    rateLimiter.executeRunnable(() -> {
      try {
        ...
        log.info("WebApp notification sent successfully: {}", message.getNotificationId());
      } catch (Exception e) {
  ...
  private void sendViaFCM(NotificationMessage message) {
    log.debug("Sending WebApp notification via FCM to: {}", message.getRecipient());
    // TODO: Implement actual FCM/WebSocket integration
  }
  ...
  private NotificationHistory saveHistory(NotificationMessage message, NotificationStatus status) {
    NotificationHistory history = NotificationHistory.builder()
        ...
        .status(status)
        .provider(com.ndk.notificationservice.enums.NotificationProvider.FCM)
  ...
  private void handleFailure(NotificationMessage message, Exception e) {
    log.error("Failed to send WebApp notification: {}", message.getNotificationId(), e);
