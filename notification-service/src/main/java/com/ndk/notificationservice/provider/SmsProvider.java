...
public class SmsProvider implements NotificationProvider {
  ...
  public void send(NotificationMessage message) {
    RateLimiter rateLimiter = rateLimiterRegistry.rateLimiter("sms");
    ...
    rateLimiter.executeRunnable(() -> {
      try {
        ...
        log.info("SMS sent successfully: {}", message.getNotificationId());
      } catch (Exception e) {
  ...
  private void sendViaTwilio(NotificationMessage message) {
    log.debug("Sending SMS via Twilio to: {}", message.getRecipient());
  ...
  private NotificationHistory saveHistory(NotificationMessage message, NotificationStatus status) {
    NotificationHistory history = NotificationHistory.builder()
        ...
        .status(status)
        .provider(com.ndk.notificationservice.enums.NotificationProvider.TWILIO)
  ...
  private void handleFailure(NotificationMessage message, Exception e) {
    log.error("Failed to send SMS: {}", message.getNotificationId(), e);
